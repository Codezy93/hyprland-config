...
APP_PACKAGES=(
    ...
    ["Neovim"]="neovim"
    ["NodeJS"]="nodejs npm"
    ["NVIDIA CUDA"]="cuda cudnn"
    ["Obsidian"]="obsidian-bin"
    ["Postman"]="postman-bin"
    ["Qalculate"]="qalculate-gtk"
    ["R + RStudio"]="r rstudio-desktop-bin"
    ["Rust"]="rustup"
...
APP_ORDER=(
    "Bitwarden" "Brave" "Claude Code" "CMake" "Cursor"
    "Docker" "Ente Auth" "Google Chrome" "Java JDK" "LocalSend"
    "Miniconda" "MongoDB Compass" "Thunderbird" "MySQL Workbench"
    "Neovim" "NodeJS" "NVIDIA CUDA" "Obsidian" "Postman"
    "Qalculate" "R + RStudio" "Rust" "Spotify"
)

SELECTED_APPS=()

echo -e "  ${BOLD}[A]${NC} Install ALL apps"
echo -e "  ${BOLD}[S]${NC} Select individually"
echo -e "  ${BOLD}[N]${NC} Skip app installation"
echo ""
read -p "  > " -n 1 -r APP_CHOICE
echo ""

case "$APP_CHOICE" in
    [Aa])
        SELECTED_APPS=("${APP_ORDER[@]}")
        info "Installing all applications."
        ;;
    [Ss])
        echo ""
        ...
        ;;
    *)
        warn "Skipping app installation."
        ;;
esac
...
if [ ${#SELECTED_APPS[@]} -gt 0 ]; then
    ...
    for app in "${SELECTED_APPS[@]}"; do
        # Split package string into array
        read -ra pkgs <<< "${APP_PACKAGES[$app]}"
        install_pkgs "$app" "${pkgs[@]}"
    done
fi

# ── Docker post-install ────────────────────────
if printf '%s\n' "${SELECTED_APPS[@]}" | grep -q "Docker"; then
    section "Docker Setup"
    if ! groups "$USER" | grep -q docker; then
        info "Adding $USER to docker group..."
        sudo usermod -aG docker "$USER"
        info "Added. You'll need to re-login for this to take effect."
    else
...
# ── Rust post-install ──────────────────────────
if printf '%s\n' "${SELECTED_APPS[@]}" | grep -q "Rust"; then
    section "Rust Setup"
